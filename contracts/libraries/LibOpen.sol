...
library LibOpen {
    ...
    function _swap(
        ...
        uint256[] memory ret;
        ret = IPancakeRouter01(PANCAKESWAP_ROUTER_ADDRESS).swapExactTokensForTokens(
            _fromAmount,
    ...
    function _getAmountOutMin(
        ...
        // same length as path
        uint256[] memory amountOutMins = IPancakeRouter01(PANCAKESWAP_ROUTER_ADDRESS).getAmountsOut(_amountIn, path);
    ...
    function _permissibleCDR(
        ...
        uint256 rF = _getReserveFactor() * _avblMarketReserves(_loanMarket);
        ...
        )
    {
        ...
        if (_commitment == _getCommitment(2)) {
            ...
            emit WithdrawCollateral(_sender, collateral.market, collateral.amount, loan.id, block.timestamp);
    ...
    function _collateralPointer(
        ...
        //if (_commitment != _getCommitment(0)) {
        require(
            (collateral.timelockValidity + collateral.activationTime) >= block.timestamp,
    ...
    function _accruedYieldCollateral(
        ...
        (cYield.oldLengthAccruedYield, cYield.oldTime, aggregateYield) = _calcAPY(
            _commitment,
    ...
    function _accruedInterest(
        ...
        deductibleUSDValue =
            (((ds.indLoanRecords[_account][_loanMarket][_commitment].amount) * _getLatestPrice(_loanMarket)) *
                aggregateYield) /
            (365 * 86400 * 10000);
        ds.indAccruedAPR[_account][_loanMarket][_commitment].accruedInterest +=
            deductibleUSDValue /
            _getLatestPrice(ds.indCollateralRecords[_account][_loanMarket][_commitment].market);
    ...
    function _transferAnyBEP20(
        ...
        address _sender,
        address _recipient,
        uint256 _value /*authContract(RESERVE_ID)*/
    ) internal {
    ...
    modifier authContract(uint256 _facetId) {
        require(
            _facetId == diamondStorage().facetIndex[msg.sig] || diamondStorage().facetIndex[msg.sig] == 0,
