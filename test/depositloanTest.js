...
describe(" Complex Test ", function () {
  ...
  it("Check Deposit", async () => {
    ...
    await expect(
      deposit.connect(accounts[1]).depositRequest(symbolUsdt, comit_NONE, depositAmount, { gasLimit: 5000000 }),
    ...
    await expect(
      deposit.connect(accounts[1]).depositRequest(symbolUsdc, comit_NONE, depositAmount, { gasLimit: 5000000 }),
    ).emit(deposit, "NewDeposit");
  ...
  it("Check loan", async () => {
    ...
    await expect(
      loanExt
        .connect(accounts[1])
        .loanRequest(symbolUsdc, comit_ONEMONTH, loanAmount, symbolUsdc, collateralAmount, { gasLimit: 5000000 }),
  ...
  it("Check addCollateral", async () => {
    ...
    await expect(
      loan
        .connect(accounts[1])
        .addCollateral(symbolUsdc, comit_ONEMONTH, symbolUsdc, collateralAmount, { gasLimit: 5000000 }),
